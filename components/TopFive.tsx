...
export async function TopFive() {
  ...
  return (
    <section className="mb-8 rounded-xl border border-edge bg-card p-4">
      ...
      <ol className="grid gap-1.5 sm:grid-cols-5">
        {rows.map((r) => (
          <a
            ...
            className="flex items-center gap-2 rounded-lg border border-edge px-3 py-2 transition hover:border-accent sm:flex-col sm:items-start sm:gap-0.5"
          >
            <span className="text-sm font-bold text-accent">#{r.rank}</span>
            <span className="min-w-0 flex-1 truncate text-sm font-semibold sm:w-full">
              {r.name}
            </span>
